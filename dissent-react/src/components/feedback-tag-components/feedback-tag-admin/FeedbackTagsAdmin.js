...
function FeedbackTagsAdmin ( { user } ) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`http://localhost:8080/api/feedback-tag/with-inactive`);
  ...
  return (
    <div>
      ...
      {feedbackTagToEdit ? <EditFeedbackTag feedbackTag={feedbackTagToEdit} setFeedbackTagToEdit={setFeedbackTagToEdit} user={user} /> : null}
      {feedbackTagToDelete ? <DeleteFeedbackTag feedbackTag={feedbackTagToDelete} setFeedbackTagToDelete={setFeedbackTagToDelete} user={user} /> : null}
      {feedbackTagToReactivate ? <ReactivateFeedbackTag feedbackTag={feedbackTagToReactivate} setFeedbackTagToReactivate={setFeedbackTagToReactivate} user={user} /> : null}
      ...
      <table className="table table-light table-striped mt-3">
        ...
        <tbody>
          {feedbackTags.filter(ft => ft.active).map(ft => makeFeedbackTagAdminRow(ft))}
        </tbody>
      </table>
